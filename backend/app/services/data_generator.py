...
def generate_mock_data(intervals: int = 96):
    """
    Generates simple temperature prediction data that increases by 1°C every 15 minutes.
    ...
    timestamps = pd.date_range(end=now, periods=intervals, freq="15T")

    # Starting temperature
    start_temp = 20

    # Temperature increases by 1°C every 15 minutes
    temperature = start_temp + np.arange(intervals)
...
# def generate_mock_data(intervals: int = 96):
#     """
#     Simulates 15-minute interval water temperature data for a smart electric water heater
